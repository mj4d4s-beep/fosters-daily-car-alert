...
export const metadata: Metadata = {
  title: "Dealer and Facebook Listings for Foster’s Car Search — September 20, 2026",
    ...
    <article className={`car-card source-${kind} ${car.rank === 1 ? "winner" : ""}`}>
      ...
      <div className="car-copy">
        ...
        <div className="dealer">
          <span>{identity}</span>
          {dealerCar ? (
            dealerCar.googleReviewsUrl ? (
              <a className="google-reviews" href={dealerCar.googleReviewsUrl} target="_blank" rel="noreferrer">
                {dealerCar.rating} · Read Google reviews ↗
              </a>
            ) : (
              <span>Google rating and reviews unavailable</span>
            )
          ) : (
  ...
  return (
    <main>
      <header className="hero">
        <nav><span className="wordmark">FOSTER’S</span><span className="date">SUNDAY · SEP 20, 2026</span></nav>
      ...
      <section className="coverage">
        ...
        <p className="coverage-note">This run rechecked every active listing and each dealer’s Google reviews. The Kia Forte fell to $4,500; it remains in the Facebook ranking but is not new today. Newly screened Facebook cars outside the search area or without enough title, VIN, maintenance or condition evidence were not added. No first-time qualifying cars survived the checks. No stock or substitute images were used.</p>
      </section>
      ...
      <footer><p>Fresh search completed September 20, 2026. Dealer and Facebook inventory and history are stored separately.</p><p>Availability changes quickly. Verify the listing, VIN, title and condition again before traveling.</p></footer>
    </main>
  );
}
